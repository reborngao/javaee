 ...
 */
package com.visabao.machine.widget.calendar;
...
public class CalendarView extends LinearLayout {
	...
	private OnCalendarItemClickListener mOnItemClickListener;
	private long selectMinDate;
	...
	private  void  initData(Context context,Calendar calendar){
		...
		width = (int) (display.getWidth()-context.getResources().getDimension(R.dimen.main_left_width));
		...
		mLinearLayoutHeader.addView(mCalendarHeader,new LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.FILL_PARENT));
	...
	public CalendarView(Context context, long startDate) {
		super(context);
		Calendar calendar=Calendar.getInstance();
		calendar.clear();
		calendar.setTime(new Date(startDate));
		initData(context,calendar);
	}
	...
	public void setOnItemClickListener(
			OnCalendarItemClickListener mAbOnItemClickListener) {
	...
	/** 点击日历，触发事件. */
	private OnCalendarItemClickListener mOnDayCellClick = new OnCalendarItemClickListener(){
		...
		public void onClick(int position) {
			...
			if(mCalendarCell.isActiveMonth()){
				...
				for(int i=0;i<mCalendarCells.size();i++){
					...
					if (mCalendarCellOther.isSelected()){
						selectPostion=i;
					}
					mCalendarCellOther.setSelected(false);
				}
				boolean isSelectMinDate=true;
				if (selectMinDate>0){
					Calendar calendar=Calendar.getInstance();
					calendar.setTime(new Date(selectMinDate));
					if (mCalendarCell.getThisCellDate().compareTo(calendar)<0){
						isSelectMinDate=false;
					}
					mCalendarCell.setSelected(isSelectMinDate);
				}
				if (!isSelectMinDate){
					if (selectPostion>-1)
					mCalendarCells.get(selectPostion).setSelected(true);
				}
	...
	public String getCalSelected() {
		...
		return iYear+"-"+ StringUtil.strFormat10(iMonth+"")+"-"+StringUtil.strFormat10(iDay+"");
	}

	public void setSelectMinDate(long selectMinDate) {
		this.selectMinDate = selectMinDate;
	}
